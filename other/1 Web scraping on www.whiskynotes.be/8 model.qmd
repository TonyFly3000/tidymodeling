...
```{python}
# print wordcloud
show_wordcloud(review_remove_stop_word)
```
...
```{python}
padded_train=padded[1:4000]
padded_test=padded[4001:-1]
```

```{python}
review_flag_final_train=review_flag_final[1:4000]
review_flag_final_test=review_flag_final[4001:-1]
```


```{python}
sum(review_flag_final_train)
```
...
# Train the model
history=model_dnn.fit(x=padded_train, y=review_flag_final_train, validation_data=(padded_test, review_flag_final_test),epochs=num_epochs,callbacks=[callbacks])
...
# Train the model
history2=model_lstm.fit(x=padded_train, y=review_flag_final_train, validation_data=(padded_test, review_flag_final_test),epochs=num_epochs,callbacks=[callbacks])
